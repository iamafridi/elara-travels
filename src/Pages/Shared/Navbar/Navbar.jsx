...
const Navbar = () => {
    ...
    const navLinks = <>
        ...
        <li>
            <Link to='/dashboard/cart'>
                <button className="btn border border-white btn-sm btn-ghost">
                    <FaShoppingCart />
        ...
        {
            user ? <>
                <button onClick={handleLogout} className="btn btn-sm btn-ghost border border-white rounded-full">Logout</button>
            </> :
                <>
    ...
    return (
        <div>
            <div className="navbar px-4 max-w-screen-xl absolute bg-opacity-30 z-10 text-white">
                <div className="navbar-start text-black">
                    <div className="dropdown">
                        <div tabIndex={0} role="button" className="btn btn-ghost bg-gray-100 text-black lg:hidden">
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h8m-8 6h16" /></svg>
                        </div>
                        <ul tabIndex={0} className="menu menu-sm dropdown-content bg-gray-500 mt-3 z-[1] p-2 shadow bg-base-900 text-white rounded-box w-52">
                            {navLinks}
                        </ul>
                    </div>
                    <a className="btn btn-ghost text-3xl font-bold "><SiYourtraveldottv className="lg:text-2xl" />Elara</a>
                </div>
                <div className="navbar-center hidden  text-gray-800 lg:flex">
